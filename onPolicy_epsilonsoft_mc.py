...
for action_idx in range(9):
    ...
    if ax > 0:  # Moving right toward x=49
        ...
        Q[action_idx] = -2.0
    if ay < 0:
        Q[action_idx, 0, :, :, 0, 0] = 20.0

#Q profile should look like moving towards the goal ideally
Q[:,10:,:,:,3:] /= 4.0
Q[:,:10,:,3:,:] /= 4.0
Q[:,:10,:,:,3:] *= 5.0
epsilon = 0.1
...
def generate_episode():
    ...
    R = []
    while True:
        # pi is to go up at the start line and close to the finish line to move right
        velocity = [0,0]
        ...
        while True:
            ...
            if race_track[new_state[0]][new_state[1]]:
                ...
                while True:
                    ...
                    if repeat:
                        #no use waiting for the probability to be good
                        act  = np.array((np.random.choice(actions), np.random.choice(actions)))
                    else:
                        act  = np.argmax(Q,axis=0)[new_state] if np.random.random() > epsilon/9.0 else np.array((np.random.choice(actions), np.random.choice(actions)))
                    repeat = True
                    if new_state[0] < 10:
