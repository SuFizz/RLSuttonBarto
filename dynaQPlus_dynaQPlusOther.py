...
# Q[3,:,:] = 10
# Q[0,:,8] = 20
time_taus = np.zeros((4,6,9), dtype=int)
for iters in range(50):
    ...
    time = 0
    while True:
        print (f"\rtime - {time}", end="", flush=True)
        ...
        if all(state + actions[action] == goal):
            act_rwd = 1
        if time < time_taus[action][state[0]][state[1]]:
            print (time, time_taus[action][state[0]][state[1]])
        rwd = act_rwd + kappa * np.sqrt(time - time_taus[action][state[0]][state[1]])
        time_taus[action][state[0]][state[1]] = time
        ...
        model[(action,tuple(state))] = (act_rwd, new_state)
        for i in range(n):
            ...
            Q[ac][st[0]][st[1]] += alpha * (rwd + gamma * np.max(Q, axis=0)[new_state[0]][new_state[1]] - Q[ac][st[0]][st[1]])
        state = new_state
        if all(state == goal):
    ...
    print (f"iters {iters} - {num_steps}")#, flush=True, end="")
plt.plot(num_steps_0, color = 'k')
...
for iters in range(10):
    ...
    num_sts= 0
    while True:
        action = np.argmax(Q + kappa * np.sqrt(time - time_taus), axis=0)[state[0]][state[1]] if np.random.random() < epsilon else np.random.randint(0,4)
        # states = [state]
        # acts = [action]
        states.append(state)
        acts.append(action)
    ...
    num_steps_1.append(num_steps)
    print (f"iters {iters} - {num_steps}")#, flush=True, end="")
plt.plot(num_steps_1, color='r')
plt.show()